package edu.uw.tcss450team2client.services;
...
//            contact = intent.getStringExtra("username");
            ...
            if (appProcessInfo.importance == IMPORTANCE_FOREGROUND || appProcessInfo.importance == IMPORTANCE_VISIBLE) {
                //app is in the foreground so send the message to the active Activities
                Log.d("PUSHY", "Contact request received in foreground: " + contact);

                //create an Intent to broadcast a message to other parts of the app.
                Intent i = new Intent(RECEIVED_NEW_CONTACT_REQUEST);
                i.putExtra("username", contact);
                i.putExtras(intent.getExtras());
                ...
                //app is in the background so create and post a notification
                Log.d("PUSHY", "Contact received in background: " + contact);
